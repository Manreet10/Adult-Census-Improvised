...
if 'Pie Chart' in plot_types:
  ...
  pie_data = census_df['income-group'].values_counts()
  plt.figure(figsize = (12,6))
  plt.title(f"Pie Chart for Income Group")
  plt.pie(pie_data,labels = pie_data.index,startangle = 30,explode = np.linspace(0.06,0.16,6))
  st.pyplot()
  
  plt.figure(figsize = (12,6))
  plt.title(f"Pie Chart for Gender")
  pie_data1 = census_df['gender'].values_counts()
  plt.pie(pie_data1,labels = pie_data1.index,startangle = 30,explode = np.linspace(0.06,0.16,6))
  st.pyplot()